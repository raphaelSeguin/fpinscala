...
object List: // `List` companion object. Contains functions for creating and working with lists.
  ...
  def drop[A](l: List[A], n: Int): List[A] = if n <= 0 then l
  else
    l match
      case Nil         => Nil
  ...
  def reverse[A](l: List[A]): List[A] =
    foldLeft(l, Nil, (reversed, element) => Cons(element, reversed))

  def appendViaFoldRight[A](l: List[A], r: List[A]): List[A] =
    foldRight(l, r, (el, acc) => Cons(el, acc))

  def concat[A](l: List[List[A]]): List[A] =
    foldLeft(l, Nil, (acc, list) => append(acc, list))
    // foldRight(l, Nil, (acc, list) => append(acc, list))
